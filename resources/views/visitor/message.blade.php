...
<br>
	<div class="container">
	<h1 style="color: #f067ae"> الرسائل الواردة</h1>
		<div class="row">
			<div class="col-sm-12">
				...
				<table class="table table-striped" id="myTable" data-freezecolumns="2" style="direction: rtl">
					<thead style="color: #f067ae">
						<tr>
							...
							<th rowspan="2">تاريخ المحادثة</th>
						</tr>
						
					</thead>
					<tbody>
@if(isset($all_conversations))
	@foreach($all_conversations as $all_conversation)
		<tr>
		    @if($all_conversation->client_1 == \Auth::guard('client')->user()->id)
			<td><a href="/client/message/{{ $all_conversation->client_2 }}">
			    {{ \App\Client::find($all_conversation->client_2)->fname . ' ' . \App\Client::find($all_conversation->client_2)->lname }}
			    </a></td>
			@else
			<td><a href="/client/message/{{ $all_conversation->client_1 }}">
			    {{ \App\Client::find($all_conversation->client_1)->fname . ' ' . \App\Client::find($all_conversation->client_1)->lname }}
			    </a></td>
			@endif
			<td>{{ date_parse($all_conversation->created_at)['year']."-".date_parse($all_conversation->created_at)['month']."-".date_parse($all_conversation->created_at)['day'] }}</td>
		</tr>
	@endforeach
@endif
					</tbody>
