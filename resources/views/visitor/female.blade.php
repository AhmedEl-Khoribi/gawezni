...
<style>
    html {
  background: radial-gradient(pink, pink) !important;
  min-height: 100%;
...
#grid { 
      display: grid;
      grid-template-columns: 300px 1fr; /* twee kolommen */ 
      grid-template-rows: 200px 200px 200px; /* drie rijen */ 
      width: 80%;
      margin: 20px auto;
      color: white;
...
#gebied0 { 
    ...
    border: 1px solid #f067ae;
    border-radius: 50px;
} 
        #gebied0 img ,#gebied1 img ,#gebied2 img {
            height: 160px;
            display: block;
    margin-left: auto;
    margin-right: auto ;
...
?>
<body style="background-color: pink !important; background-size: cover; font-family: 'Aref Ruqaa', serif;"  >
    <!--start-home-->
    <div id="home" class="header">
        <div class="header-top">
            <div class="container">
                <div class="logo">
                    <a href="/"><img src="/site_info/{{$setting->logo}}"></a>
                </div>
...
<div class="container-fluid">
    <div class="row">
        <div class="col-lg-3 col-md-3 hidden-sm hidden-xs pic">
        <div id="grid"> 
      @php
        $users = \App\Client::orderByRaw('RAND()')->where('gender', 'male')->take(3)->get();
      @endphp
      @for($i=0; $i<=2; $i++)
      <div id="gebied{{$i}}"><a href="/client/profile/{{ $users[$i]->id }}"> <img src="/files/users/{{ $users[$i]->image }}" width="200px"> </a></div>
      @endfor
        </div>
        </div>
  ...
  <div class="modal-bodies">
    <div class="modal-body modal-body-step-1 is-showing">
      <div class="title">الخطوه الاولي</div>
      <div class="description">اهلا بك ,  لتبدا التسجيل</div>
      <form  enctype="multipart/form-data" action="{{ url('femaleRegister') }}" method="post">
          {{ csrf_field() }}
        ...
        <input type="text" name="lname" placeholder="الاسم الاخير"/>
         ...
         <input type="password" id="repassword" name="password_confirmation" placeholder="اعادة كلمة السر"/>
         <hr>
    ...
    <div class="modal-body modal-body-step-2">
      <div class="title">الخطوه الثانيه</div>
        <input type="email" name="email" placeholder="الايميل"/>
        <label>تاريخ الميلاد</label>
         <input type="date" name="dob" placeholder="تاريخ الميلاد "/>
         <label> ضع صورتك</label>
         <input type="file" id="my-file" name="image" placeholder="صوره"/>
    ...
    <div class="modal-body modal-body-step-3">
      <div class="title">الخطوة الثالثة</div>
      <div class="description">من فضلك انتظر تفعيل الحساب</div>
      <div class="text-center">
        <button type="submit" class="button">تم!</button>
            <br>
        <center>
            <div class="rerun-button">مراجعة بياناتك</div>
        </center>
      </div>
    </form>
    </div>
        ...
        @php
        $uss = \App\Client::orderByRaw('RAND()')->where('gender', 'male')->take(3)->get();
        @endphp
        <div class="col-lg-3 col-md-3 col-xs-12 col-sm-12 ">
            <div id="grid"> 
     @for($i=0; $i<=2; $i++)
      <div id="gebied{{$i}}"><a href="/client/profile/{{ $uss[$i]->id }}"> <img src="/files/users/{{ $uss[$i]->image }}" width="200px"> </a></div>
      @endfor
            </div>
        </div>
    </div>
