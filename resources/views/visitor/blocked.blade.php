    ...
    <div class="container">
        @if($flash = session('message'))
          <div class="alert success">
              ...
              <b  style="direction:rtl; float:right;">{{ $flash }}</b>
          </div>
      @endif
            ...
            <div class="col-sm-12">
                ...
                <table class="table table-striped" id="myTable" data-freezecolumns="2" style="direction: rtl">
                    <thead style="color: #f067ae">
                        <tr>
                            <th>#</th>
                            <th colspan="2">اسم المستخدم</th>
                            <th>تاريخ الحظر</th>
                            <th>الغاء الحظر</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($in_friend as $b)
                        <tr>
                            <td>{{$i++}}</td>
                            <td><a href="/client/profile/{{ $b->friend_id }}">{{ $b->username }}</a></td>
                            <td><img src="/files/users/{{ $b->image }}" width="50px"></td>
                            <td style="direction:ltr">{{ $b->created_at->diffForHumans() }}</td>
